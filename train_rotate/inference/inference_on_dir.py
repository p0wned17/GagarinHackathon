...
# Функция для преобразования шестнадцатеричного цвета в BGR
...
# Загрузка модели
def load_model():
    model = torch.jit.load("bestv5.torchscript", map_location="cuda:0")
...
# Слайдинг-виндоу инференс
def sliding_inference(image, model):
...
def process_and_visualize_image(image_path, output_folder, model, classes):
    ...
    t1 = time.time()
    predictions = sliding_inference(image, model)
    print(time.time() - t1)
...
def process_csv(csv_path, output_folder, model, classes):
    df = pd.read_csv(csv_path)
    for image_path in df["image_path"]:
        process_and_visualize_image(image_path, output_folder, model, classes)


if __name__ == "__main__":
    csv_path = "/home/cv_user/projects/hackathon/pipe_detect/output.csv"
    output_folder = "./inference_visualizations_v5"
    model = load_model()
    classes = load_classes()
    process_csv(csv_path, output_folder, model, classes)
